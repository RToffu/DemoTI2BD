...
//--> CONTSTRUTOR
	...
	public boolean inserirProduto(Produto produto) {
		...
		try {
			...
			st.executeUpdate("INSERT INTO produto (id, device_name, device_category, qnt) " + "VALUES (" + produto.getId()
					+ ", '" + produto.getDevice_name() + "', '" + produto.getDevice_category() + "', '" + produto.getQnt() + "');");
			st.close();
			status = true;
	...
	public boolean atualizarProduto(int id, Scanner scan) {
		...
		try {
			...
			String sql = "UPDATE produto SET device_name = '" + produto.getDevice_name() + "', device_category = '" + produto.getDevice_category()
					+ "', qnt = '" + produto.getQnt() + "'" + " WHERE id = " + produto.getId();
		...
		try {
			...
			if (rs.next()) {
				...
				for (int i = 0; rs.next(); i++) {
					produtos[i] = new Produto(rs.getInt("id"), rs.getString("device_name"), rs.getString("device_category"),
							rs.getInt("qnt"));
				}
			}
			st.close();
